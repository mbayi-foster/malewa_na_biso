  ...
  <form class="max-w-sm mx-auto mb-5" @submit.prevent="handleSubmit">
    ...
    <div>
      ...
      <p class="text-center mb-3 text-sm font-normal text-gray-500 lg:text-lg dark:text-gray-400">
        Veillez entrer les informations du nouvelle utilisateur
      </p>
    </div>
    <div class="mb-5">
    ...
    <div class="mb-5">
      <label for="capitale" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white"
        >Capitale</label
      >
      <input
        v-model="user.toPaye"
...
const handleSubmit = async () => {
  ...
  try {
    loading.value = true
    await Users.post('/users', newUser)
    msg.value = 'Succès'
    router.push('/users')
  } catch (error) {
    loading.value = false
