  ...
  <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
    <div class="flex justify-between items-center">
      ...
      <button
        ...
        title="Actualiser"
        @click="refresh()"
        class="text-center rounded-full text-yellow-400 hover:text-white border border-yellow-400 hover:bg-yellow-500 focus:ring-4 focus:outline-none focus:ring-yellow-300 font-medium text-sm w-12 h-12 me-2 mb-2 dark:border-yellow-300 dark:text-yellow-300 dark:hover:text-white dark:hover:bg-yellow-400 dark:focus:ring-yellow-900"
      >
      ...
      <tbody v-if="hasData && !load">
        <tr v-for="(item, index) in paginatedData" :key="item.id" scope="row" class="px-6 py-4">
          ...
          <td v-for="column in columns" :key="column.key">
            <div
              v-if="
                column.key != 'date_depot' &&
                column.key != 'photo' &&
                column.key != 'comptable' &&
                column.key != 'pdg'
              "
            >
              <p v-if="column.key == 'montant'">
                {{ numeral(item[column.key]).format('0,0') }} Fc
              </p>
              <p v-else>{{ item[column.key] }}</p>
            </div>
            <div v-if="column.key == 'date_depot'">
              <p>{{ format(item['date_depot'], 'dd MMMM yyyy', { locale: fr }) }}</p>
            </div>
            <div class="text-center flex justify-center" v-if="column.key == 'photo'">
              <img class="w-10 h-10 rounded-full" src="@/assets/logo.svg" alt="user photo" />
            </div>
            <div v-if="column.key == 'comptable'">
              <p v-if="item['comptable']" class="text-green-500">Validé</p>
              <p v-else class="text-red-600">En cours</p>
            </div>
            <div v-if="column.key == 'pdg'">
              <p v-if="item['pdg']" class="text-green-500">Validé</p>
              <p v-else class="text-red-600">En cours</p>
            </div>
          </td>
        </tr>
    ...
    <p
      v-if="!hasData && !load"
      class="text-center mb-3 text-lg text-red-500 md:text-xl dark:text-gray-400"
    >
...
const formattedAmount = numeral(amount).format('$0,0.00') // "$1,234,567.89"
